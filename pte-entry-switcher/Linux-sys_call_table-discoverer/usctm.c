...
static inline void usctm_trampoline(struct pt_regs *regs, unsigned int nr){
...
unsigned char x64_sys_call_orig[INST_LEN];
...
int init_module(void) {
	...
	if (register_kprobe(&kp_x64_sys_call)) {
        ...
        offset = (unsigned long)usctm_trampoline - x64_sys_call_addr - INST_LEN;
...
//usage of the syscall table while dispatching syscalls
	memcpy(x64_sys_call_orig, (void*)x64_sys_call_addr, INST_LEN); //save original bytes to be overwritten for unmount module issues
...
void cleanup_module(void) {
    ...
    if (x64_sys_call_addr)
        memcpy((void*)x64_sys_call_addr, x64_sys_call_orig, INST_LEN);

    if (hacked_syscall_tbl && hacked_ni_syscall)
        hacked_syscall_tbl[FIRST_NI_SYSCALL] = (unsigned long*)hacked_ni_syscall;
    end_syscall_table_hack();

    printk("%s: module unloaded safely\n", MODNAME);
}
#else
void cleanup_module(void) {
