...
void read_csv_file(double** data, double* y_temp, double** y, char* dataset){
    ...
    // *** Salta la riga di intestazione (Header Skip) ***
    if(fgets(buffer, sizeof(buffer), file) == NULL) {
        printf("Error: Could not read header row from CSV file.\n");
        fclose(file);
        exit(1);
    }

    int max_samples = (strcmp(dataset, "train") == 0) ? N_SAMPLES : N_TEST_SAMPLES; // Ottieni la dimensione massima
    
    int i = 0;
    while(fgets(buffer, sizeof(buffer), file)){
        
        // *** AGGIUNGI QUI: Controllo di sicurezza ***
        if (i >= max_samples) {
            printf("Warning: CSV file contains more rows than allocated (%d). Stopping read.\n", max_samples);
            break; 
        }
        ...
        while(token != NULL){
            if(j == 0){
                ...
                data[i][j-1] = (double)atoi(token); // <-- LINEA 34 (ora protetta)
            }
            j++;
            token = strtok(NULL, ",");
        }
    ...
    if(strcmp(dataset, "train") == 0){
        ...
        printf("Error: Invalid dataset type '%s' in read_csv_file. Exiting.\n", dataset);
        exit(1); 
    }
    for(int i=0;i<total_samples;i++){ //if neither test or train this loop is not executed → no dataset
